...
TxNode(i) ≜
    Group(
        ⟨
            ...
            Text(8, 18, "Tx " ∘ ToString(i), ("fill" :> "black" @@ "font-size" :> "12" @@ "font-family" :> "monospace")),
            Text(8, 34, StatusLabel(execStatus[i]), ("fill" :> "black" @@ "font-size" :> "11" @@ "font-family" :> "monospace")),
            Text(8, 50, "inc=" ∘ ToString(incarnation[i]), ("fill" :> "black" @@ "font-size" :> "11" @@ "font-family" :> "monospace"))
        ⟩,
        [transform ↦ "translate(" ∘ ToString(16 + 132 * (i - 1)) ∘ ",20)"]
    )

TxNodes ≜ [i ∈ TxIndex ↦ TxNode(i)]
...
AnimView ≜
    Group(⟨Legend⟩ ∘ TxNodes, [transform ↦ "scale(1.0)"])
