...
Merge(r1, r2) ≜
    LET D1 ≜ DOMAIN r1
        D2 ≜ DOMAIN r2
    IN [k ∈ (D1 ∪ D2) ↦ IF k ∈ D1 THEN r1[k] ELSE r2[k]]
...
ExecRows ≜ IF Executors = 0 THEN 0 ELSE ((Executors + ExecCols - 1) ÷ ExecCols)
...
HeaderElems ≜
    ⟨
        ...
        Text(16, 40,
             "exec_idx=" ∘ ToString(execution_idx)
                ∘ "  val_idx=" ∘ ToString(validation_idx)
                ∘ "  active=" ∘ ToString(active_tasks)
                ∘ "  wave=" ∘ ToString(validation_wave),
...
ExecutorCard(e) ≜
    Group(
        ⟨
            ...
            Text(10, 20, "Executor " ∘ ToString(e), ("fill" :> "black" @@ "font-size" :> "12" @@ "font-family" :> "monospace")),
            Text(10, 40, "task: " ∘ TaskLabel(tasks[e]), ("fill" :> "black" @@ "font-size" :> "11" @@ "font-family" :> "monospace")),
            Text(10, 58, "terminated: " ∘ ToString(terminated[e]), ("fill" :> "black" @@ "font-size" :> "11" @@ "font-family" :> "monospace"))
        ⟩,
        [transform ↦ "translate(" ∘ ToString(ExecGridLeft + ExecColStep * ((e - 1) % ExecCols)) ∘ "," ∘ ToString(ExecGridTop + ExecRowStep * ((e - 1) ÷ ExecCols)) ∘ ")"]
    )

ExecutorCards ≜ [e ∈ 1‥Executors ↦ ExecutorCard(e)]

TxStatusBox(txn) ≜
    Group(
        ⟨
            Rect(0, 0, 100, 48, [fill ↦ TxStatusColor(execStatus[txn]), stroke ↦ "black", rx ↦ 6]),
            Text(8, 18, "tx=" ∘ ToString(txn), ("fill" :> "black" @@ "font-size" :> "11" @@ "font-family" :> "monospace")),
            Text(8, 34, execStatus[txn], ("fill" :> "black" @@ "font-size" :> "10" @@ "font-family" :> "monospace")),
            Text(8, 46, "inc=" ∘ ToString(incarnation[txn]), ("fill" :> "black" @@ "font-size" :> "10" @@ "font-family" :> "monospace"))
        ⟩,
        [transform ↦ "translate(" ∘ ToString(TxGridLeft + TxColStep * ((txn - 1) % TxCols)) ∘ "," ∘ ToString(TxGridTop + TxRowStep * ((txn - 1) ÷ TxCols)) ∘ ")"]
    )

TxStatusBoxes ≜ [txn ∈ Tx!TxIndex ↦ TxStatusBox(txn)]
...
AnimView ≜ Group(HeaderElems ∘ ExecutorCards ∘ ⟨Legend⟩ ∘ TxStatusBoxes, [transform ↦ "scale(1.0)"])
